...
def start():
        ...
        print ("   ")
        print ("   ")
        print ("This Bot Is Created By [Dark]Lord#5695")
        print ("This is skidded from sources like gorilla and nihal's bot.")
        print ("I updated it soo...")
        print ("   ")
        print ("   ")
        accounts_created = 0
        accounts_joined = 0
        ...
        class myThread(threading.Thread):
                ...
                def __init__(self, username=None, proxies=None, group=None, favorite=None, follow=None):
                        ...
                        self.password = "darkclient101"
                        self.email = "{0}@Darky101.com".format(self.username)
                ...
                def favorite_item(self, amount = 1):
                        def retry():
                                ...
                                if amount < 5:
                                        try:
                                                ...
                                                print("Unnamed error".format(self, str(e2)))
                                else:
                                        pass
                        ...
                        if r.text == "{\"success\":false,\"message\":\"Whoa. Slow down.\"}":
                                print("{0.username} going too fast, waiting 30 seconds.".format(self))
                                sleep(30)
                                retry()
                                return False
                ...
                def create(self, amount = 1):
                        ...
                        try:
                                ...
                                if userID:
                                        ...
                                        self.successfully_created = True
                                        print("Successfully created a account {0.username}:{0.password}:{0.email}:{0.userID}:{1}".format(self, (self.proxy["https"] or "None").replace("https://", "").replace(":", ",")))
                                ...
                                if r.json()["reasons"][0] == "Captcha":
                                        print("{0.username} -> CAPTCHA, is blocking.".format(self))
                                        self.successfully_created = False
                                        retry()
                                        return
                                else:
        ...
        def scrape():
                s = requests.Session()
                print("Stealing proxies...")
                proxies_urls = ["http://www.live-socks.net/2018/03/13-03-18-socks-5-servers-8360.html#more",
                                                "http://www.live-socks.net/2018/03/13-03-18-socks-5-servers-8360.html#more"]
                proxies = []
                r = s.get(url = "http://www.live-socks.net/2018/03/13-03-18-socks-5-servers-8360.html#more")
                ...
                print("   ")
                print("   ")
                print("Starting to create/favorite")
                print("This ussualy takes 5-30 seconds")
                print("   ")
                return proxies
        ...
        with open("config.txt", "r") as config:
                config = config.read()
        restart=print("Do you want to start again?")
        if "restarts=yes" in config:
                start()
        else:
                if "restarts=no" in config:
                        exit()
